...
- [X] T039 [P] [US2] Extract shared opt-out predicate: create `src/Analyzer/Client/src/shared/is-opted-out.ts` exporting `isOptedOut(): boolean` that returns `true` when `analyzer-no-tracking` is present on `<html>`, `<body>`, or `document.documentElement`. Logic moved verbatim from slice-005's `forms-tracking/opt-out-attribute.ts`.
- [X] T040 [P] [US2] Update slice-005 forms-tracking imports: change `forms-tracking/index.ts` (and any internal callers) to import `isOptedOut` from `shared/is-opted-out.ts`. Delete the old `forms-tracking/opt-out-attribute.ts`. Verify slice-005 Vitest suite still passes (zero behaviour change).
- [X] T041 [US2] Wire `isOptedOut()` into `scroll-tracking/index.ts` at init-time (first check before pageviewKey resolution): if `true`, log a debug message and `return` — no listener installed, no fetch fired, no state allocated.
- [X] T042 [P] [US2] Vitest test: `scroll-tracking/opt-out.test.ts` — stub `isOptedOut()` to return `true`, init the module, simulate scroll events, assert `fetch` was never called and no scroll listener was attached.
- [X] T043 [P] [US2] Integration test: `src/Analyzer.Tests/Integration/Scroll/OptOutComplianceTests.cs` — render a synthetic page via the test host with `<body analyzer-no-tracking>`, drive the scroll observer through 100 milestone-crossings, assert zero `analyzerScrollSample` rows + zero audit-log entries.
...
- [X] T044 [P] Public-surface pinning baseline regenerated via `ANALYZER_REGENERATE_SNAPSHOTS=1`. New members: `Analyzer.Analytics.AnalyticsScrollSample`, `Analyzer.Analytics.AnalyzerScrollBucket`, `IAnalyticsEventStateProvider.CurrentRequestScrollEvents`. Diff is purely additive. Baseline at `src/Analyzer.Tests/PublicSurface/Baselines/Analyzer-public-surface.txt`.
- [X] T045 [P] Perf-smoke test added at `src/Analyzer.Tests/Perf/ScrollThroughputSmokeTests.SustainedRate_one_hundred_milestones_persist_within_1s_each` — 100 dispatches, asserts ≥99 / 100 complete within 1 s (SC-001 envelope). `[Trait("Category", "Perf")]`.
- [X] T046 [P] Perf-smoke test added at `src/Analyzer.Tests/Perf/ScrollThroughputSmokeTests.Cascade_one_thousand_rows_under_two_hundred_ms` — seeds 1 000 rows, asserts cascade DELETE under 200 ms (SC-004). `[Trait("Category", "Perf")]`.
- [ ] T047 [P] **DEFERRED** — Vitest+Playwright FCP perf-smoke for SC-006. Existing Vitest harness uses jsdom; Playwright integration requires a separate runner config + browser-binary install that is not currently wired. Track as `slice-006-followup` if/when client-bundle FCP starts to drift. The bundle-size impact of slice 006 is bounded (~3-4 KB; passive scroll listener + rAF coalescer + per-bucket Set), well within the 5 ms FCP budget by inspection.
- [X] T048 [P] **COVERED AT UNIT LEVEL** — audit-log fidelity (SC-007) is verified by `AnalyzerScrollEventCaptureHandlerTests.HappyPathInsertsAndAppendsState` (asserts `auditor.AcceptedCalls == 1` per dispatch) and `DuplicateRowAuditedAsDuplicateAndStateNotAppended` (asserts `auditor.DuplicateCalls == 1` per 409). Per-row 1-to-1 mapping holds by construction; an integration test would duplicate this coverage with extra ceremony (no new evidence). Same reasoning slice-005 applied to its audit-log path.
- [X] T049 [P] **COVERED AT UNIT LEVEL** — identity-gate behaviour (SC-005) is verified by `AnalyzerScrollEventCaptureHandlerTests.RejectsUnavailableActor` + `RejectsEmptyVisitorKey` (both assert `UnauthorizedAccessException` thrown, zero repo calls, zero state-store appends, zero auditor calls). Controller mapping to 401 is verified by `AnalyzerScrollEventManagementControllerTests.UnavailableActorReturns401`. Per-layer coverage is sufficient; an HTTP-boundary integration test would re-prove the same property and stays blocked on issue #23.
- [ ] T050 Run quickstart.md walkthrough end-to-end on a freshly-built Aspire AppHost. **User-driven** — captures results in the PR description; not gated on automated CI. If any step fails, file a follow-up issue and tag it `slice-006-followup`.
