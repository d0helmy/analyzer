...
- [ ] T038 [P] [US2] Integration test `src/Analyzer.Tests/Integration/CustomEvents/CascadeHardDeleteTests.cs` (US2 AS1) — end-to-end through `AnonymizeVisitorProfileCommand`; assert A's `analyzerCustomEvent` rows = 0; B's untouched; A's receipts also gone (slice-002); A's sessions soft-anonymised (slice-003); visitor row anonymised (slice-007).
...
- [ ] T044 [P] [US3] Integration test `src/Analyzer.Tests/Integration/CustomEvents/ValidationAndAuditTests.cs` (US3 AS1-AS4, SC-005/006/007) — five scenarios via `[Theory]`: (a) anonymous POST → 401 + 0 rows + 0 audit entries; (b) empty category → 400 + 0 rows; (c) `Length > 64` action → 400 + 0 rows; (d) NaN value → 400 + 0 rows; (e) well-formed POST → 202 + 1 row + 1 audit entry containing UPN + oid + eventKey.
